...
const TokenBalances = () => {
  ...
  const tokensNumText = tokensInfo.num > 0 ?
    `${ prefix }${ tokensInfo.num } ${ tokensInfo.num > 1 ? t('tokenBalances.tokens') : t('tokenBalances.token') }` :
    '0';

  return (
    <Flex columnGap={ 3 } rowGap={ 3 } mt={{ base: '6px', lg: 0 }} flexDirection={{ base: 'column', lg: 'row' }}>
      <TokenBalancesItem
        name={ t('tokenBalances.netWorth') }
      ...
      />
      <TokenBalancesItem
        name={ `${ currencyUnits.ether } ${ t('tokenBalances.balance') }` }
